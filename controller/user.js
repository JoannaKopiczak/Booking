              ...
              .then(() => {
                ...
                transporter
                  .sendMail({
                    ...
                    to: `${email}`,
                    subject: "Welcome to Bookin Spectacle",
                    text: `Hello Dear ${email}`,
                    html: `<b>Hello Dear User, we are happy that you join our family. Kind Regards, Booking Spectacle Team.</b>`,
                  })
      ...
      bcrypt.compare(password, user[0].password, (_err, result) => {
        if (_err) res.status(401).json({ error: "Authentication has failed!" });
          ...
          const userData = {
            email: user[0].email,
            ID: user[0]._id,
            favouriteSpectacles: user[0].favouriteSpectacles,
          };
